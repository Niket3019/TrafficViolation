    ...
    for box in boxes:
        ...
        if label >= 0:
            ...
            if tf:
                ...
                image_path = os.path.join(settings.MEDIA_ROOT, 'uploads', f'vio{dcnt}.jpg')
                cv2.imwrite(image_path, cimg)

                # Save to Django Model
                with open(image_path, 'rb') as img_file:
                    detected_image = UploadedImage()
                    detected_image.image.save(f'vio{dcnt}.jpg', File(img_file))
                    detected_image.save()
